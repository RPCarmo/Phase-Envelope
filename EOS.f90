...
module EOS
  ...
  subroutine EoS_param(acentric, Tc, ac, a, T, comp)
    ...
    do i = 1 , comp
        ...
        !if(acentric(i) .le. 0.491d0) then
        !  K = 0.37464d0 + (1.54226d0 - 0.26992d0*acentric(i))*acentric(i)
        !else
        !  K = 0.379642d0 + (1.487503d0 + (-0.164423d0 + 0.016666d0*acentric(i))*acentric(i))*acentric(i)
        !endif
  ...
  subroutine VdW1fMIX(comp, a, b, kij, lij, MoleFrac, amix, bmix) 
    
    !Arguments
    real(8) :: a(:), b(:), kij(:,:), lij(:,:), MoleFrac(:), amix, bmix
    integer :: comp
    ...
    do i = 1, comp
      do j = 1, comp        
        ...
        bmix = bmix + MoleFracAux(i)*MoleFracAux(j)*((b(i)+b(j))/2.0d0)*(1.d0 - lij(i,j))
      enddo
    enddo
...
! //////////////////////////
  ...
  subroutine fugacity(comp, T, P, a, b, amix, bmix, FugCoef, Volume, MoleFrac, kij, lij, index)
   
    !Arguments
    real(8) :: T, P, a(:), b(:), amix, bmix, FugCoef, Volume, MoleFrac(:), kij(:), lij(:)
    integer :: index, comp
    
    !Local
    real(8) :: aux, MoleFracAux(comp), sigma_eos, epsilon_eos, da_dx, db_dx, Z, R !R = cm³.bar/(mol.K)
    integer :: i
    ...
    do i = 1, comp
      ...
      db_dx = db_dx + MoleFracAux(i)*(b(i)+b(index))*(1.0d0 - lij(i))
    enddo
    
    !ln(Fugacity coefficient)
    FugCoef = ((db_dx-bmix)/bmix)*(Z-1.0d0)-dlog((Volume-bmix)*aux)-amix/(bmix*R*T*(epsilon_eos-sigma_eos))*(da_dx/amix-(db_dx-bmix)/bmix)*&
